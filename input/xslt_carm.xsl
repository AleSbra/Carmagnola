...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="mode" select="'writing_ma'"/>
            ...
            <xsl:result-document href="../output/{$mode}/{$op}-{$mode}.svg">
                <svg xmlns:cc="http://creativecommons.org/ns#"
                    ...
                    <metadata>
                        ...
                        <xsl:choose>
                            <xsl:when test="$mode = 'reading_cc'">
                                <xsl:for-each
                                    select="ancestor::tei:TEI/tei:teiHeader//tei:listChange[@xml:id = 'reading_cc']">
                                    <listChange xmlns="http://www.tei-c.org/ns/1.0">
                                        <xsl:for-each select="tei:change[@xml:id]">
                                            <xsl:if
                                                test="
                                                    if ((for $c in ancestor::tei:TEI//tei:surfaceGrp[@xml:id = $surface-id]//tei:zone/tokenize(@change, '#')
                                                    return
                                                        normalize-space($c) = current()/@xml:id) = true()) then
                                                        true()
                                                    else
                                                        false()">
                                                <change ref="{@xml:id}">
                                                  ...
                                                  <xsl:value-of
                                                  select="
                                                                    ancestor::tei:TEI//tei:surfaceGrp[@xml:id = $surface-id]//tei:zone[if ((for $c in tokenize(@change, '#')
                                                                    return
                                                                        normalize-space($c) = current()/@xml:id) = true()) then
                                                                        true()
                                                                    else
                                                                        false()]/parent::tei:surface/@n"
                                                  />
                                                  </xsl:attribute>
                                                  </xsl:if>
                            ...
                            <xsl:when test="$mode = 'reading_ma'">
                                <xsl:for-each
                                    select="ancestor::tei:TEI/tei:teiHeader//tei:listChange[@xml:id = 'reading_ma']">
                                    <listChange xmlns="http://www.tei-c.org/ns/1.0">
                            ...
                            <xsl:when test="$mode = 'writing_cc'">
                                <xsl:for-each
                                    select="ancestor::tei:TEI/tei:teiHeader//tei:listChange[@xml:id = 'writing_cc']">
                                    <listChange xmlns="http://www.tei-c.org/ns/1.0">
                                        <xsl:for-each select="tei:change">
                                            <xsl:if
                                                test="
                            ...
                            <xsl:when test="$mode = 'writing_ma'">
                                <xsl:for-each
                                    select="ancestor::tei:TEI/tei:teiHeader//tei:listChange[@xml:id = 'writing_ma']">
                                    <listChange xmlns="http://www.tei-c.org/ns/1.0">
                                        <xsl:for-each select="tei:change">
                                            <xsl:if
                                                test="
                                        ...
                                        <polygon points="{@points}" id="{@xml:id}">
                                            <xsl:attribute name="style">
                                                ...
                                                <xsl:choose>
                                                  ...
                                                  <xsl:when
                                                  test="//tei:listChange/tei:change[@xml:id = $ma_g][@type = 'txt']">
                                                  ...
                                                  <xsl:when
                                                  test="//tei:listChange/tei:change[@xml:id = $ma_g][@type = 'ptxt']">
                                                  <xsl:text>fill:#F0E68C;stroke:#ff0000;stroke-width:1.28400004;stroke-miterlimit:4;stroke-opacity:1;stroke-dasharray:5;fill-opacity:0.5;</xsl:text>
                                                  </xsl:when>
                                                  <xsl:when
